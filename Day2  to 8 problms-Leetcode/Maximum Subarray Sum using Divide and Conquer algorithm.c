...
the inner loop finds the maximum possible sum
with first element picked by outer loop and
...
Using Divide and Conquer approach,
 we can find the maximum subarray sum in O(nLogn) time.
...
We can easily find the crossing sum in linear time.
 The idea is simple,
 find the maximum sum starting from mid point and ending at some point
on left of mid, then find the maximum sum starting from mid + 1
and ending with sum point on right of mid + 1.
 Finally, combine the two and return. */

 // A Divide and Conquer based program for maximum subarray  Θ(nLogn)
// sum problem
#include <limits.h>
#include <stdio.h>
